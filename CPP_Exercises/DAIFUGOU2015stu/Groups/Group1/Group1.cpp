...
bool Group1::approve(const GameState & gstat) {
...
bool Group1::follow(const GameState & gstat, CardSet & s) {
  ...
  for(i=0;i<=gstat.nofPlayers;i++) {
    if(gstat.nofCards[i] < numoftehuda) {
      markedPlayerNum++;
  ...
  if(!syoban) { //初盤じゃない(手札４枚以下のプレイヤが居る)
    ...
    sameCardNum = numofSameCard(inHand().at(0)); //そのペア以外のものが探索対象
  ...
  switch(pileCardNum) {
    case 0: //自分が親となった（=場札がない）時
      if(syoban) {
        i = inHand().size() - 1;
        sameCardNum = numofSameCard(inHand().at(i));
        ...
        for(k=4;k>1;k--) {
          for(i=inHand().size()-1;i>0;i--) { //ジョーカーも含めて出す場合
            if(cardLessThan(pile.at(0), inHand().at(i))) {
              sameCardNum  = numofSameCard(inHand().at(i));
    ...
    case 1:
      bafuda = pile.at(0); //場札リストの最初（場札の強いカード”1枚”）をbafudaにいれる
...
int Group1::numofSameCard(const Card & card) {
  ...
  for(i=0;i<inHand().size();i++) {
    if(!cardEquals(inHand().at(i), card)) continue;
    num++;
  }
  return num;
}
