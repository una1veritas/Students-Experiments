...
bool n231054s::follow(const GameState & gstat, CardSet & s){
...
bool n231054s::is2PareinHand(CardSet & s, int & pos){
...
bool n231054s::is3PareinHand(CardSet & s, int & pos){
...
bool n231054s::is4PareinHand(CardSet & s, int & pos){
...
int n231054s::getPosbyPile(CardSet & pile){
  Card tmp;
  int pos;
  pos = -1;
  if(!pile.isEmpty()){
...
bool n231054s::cardLessThan(const Card & c1, const Card & c2) {
  int i,j;
  i = c1.getNumber();
  j = c2.getNumber();
  if(i < 3 && 0 < i)
...
bool n231054s::cardEquals(const Card & c1, const Card & c2) {
	return !cardLessThan(c1, c2) && !cardLessThan(c2, c1);
}
void n231054s::sortInHand(void) {
 ...
 void n231054s::sortInTrump(void){
...
bool n231054s::approve(const GameState & gstat){
  ...
  for(int i = 0; i < gstat.nofPlayers; i++)
    MenbersCardNum += gstat.nofCards[i]; 
  return true;
}
