...
//使用されたカードをmemory記憶する
bool Group7::approve(const GameState & gstat){
...
bool Group7::follow(const GameState & gstat, CardSet & s) {
	CardSet pile(gstat.pile);
	Card tmp, tmp2;
	...
	if(pile.size() != 0){//親でない時
		...
		//場のカードよりも強く、場に出ている枚数よりも多く持っているカードを探す
	    		if(hand[i].isGreaterThan(tmp2) && cardNum(hand[i].getNumber(), hand) >= pile.size()){
		...
		//ことができるならjokerとその次に強いカードをペアとして出す
	  	if(hand[cardNum(hand[0].getNumber(), hand)].equal(hand[hand.size()-2]) && checkSt2(hand[hand.size()-2].getNumber()) && hand[hand.size()-1].isJoker() && pile.size() == 2){
	    		c2 = hand.size()-2;
	    		for(int j = 0; j < 2; j++){
		...
		if(hand[num].equal(hand[hand.size()-1]) && checkSt(hand[hand.size()-1].getNumber()) && pile.size() == 1 && hand[hand.size()-1].isGreaterThan(tmp2))
	    		i = hand.size()-1;
		...
		//手札が4枚以上で最強カードを選んでいたら確実に親が取れない限りパス
	   	if(hand[i].equal(hand[hand.size()-1]) && !checkSt(hand[i].getNumber()) && hand.size() > 3){
		     	printf("俺はあえて出さない!");
		       	return true;
	   	}
		//
		...
		//2番めに強いカードは手札が5枚以下になるまで温存。但し右から2番めと指定しているので最強カードと同じ数の場合はある。
	   	if(hand[hand.size()-2].equal(hand[i]) && hand.size() >= 6)
		     	return true;
		
		
		//手札が3枚か4枚より多いとき(ここはランダムに決まる)に最強カードを選んでいて、最強カードの次に強いのが8以下の場合はパス
	   	if(hand[i].equal(hand[hand.size()-1]) && hand[i-1].getNumber() < 9 && hand.size() > getRandom(3,4)){
	  	...
	  	if(hand[cardNum(hand[0].getNumber(), hand)].equal(hand[hand.size()-1]) && checkSt(hand[hand.size()-1].getNumber())){//自分の最強カードを使うことで親がまたとれて、次のターンで上がれるならば最強カードを出す
		    	inHand().pickup(tmp, hand.size()-1); 
		     	s.insert(tmp);
		     	return true;
	  	}
	  	else if(hand[cardNum(hand[0].getNumber(), hand)].equal(hand[hand.size()-2]) && checkSt2(hand[hand.size()-2].getNumber()) && cardEquals(hand[hand.size()-2], hand[hand.size()-1])){//上のやつの最強カードが2枚ペアである場合
		    	c = 2;
		    	c2 = hand.size()-2;
		    	for(int j = 0; j < c; j++){
		      		inHand().pickup(tmp, c2); 
...
void Group7::memorysort(void) {
	for(int i = 0; i+1 < memory.size(); i++) {
		for(int j = i+1; j < memory.size(); j++) {
		  if (memory[i].isGreaterThan(memory[j])) {
				Card t = memory[i];
				memory[i] = memory[j];
				memory[j] = t;
			}
