...
private:
	int number;
	//0~12がスペードの1~13,13~25がダイヤモンドの1~13,以下同様に26~38がハート,39~51がクラブとして,52をジョーカーと置く。
// メンバ関数
public:
	// デフォルトコンストラクタ(初期値不定)
	Card(void)	{ }
	...
	int gsuit(void)	{
		  ...
		  return 1;
		else if(26<= number && number <39)
		  return 2;
		else if(39<= number && number <52)
		  return 3;
		else if(number == 52)
		  return 4;
		else
