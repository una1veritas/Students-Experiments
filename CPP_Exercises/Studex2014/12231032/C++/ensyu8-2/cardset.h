...
//	作者: 核雅瑛(あなたの名前); 日付2014/04/17: (完成した日付)
//
#ifndef CARDSET_H
#define CARDSET_H
...
/*********************************
*0~12:spade, 13~25:diamond, 
*26~38:heart, 39~51:club 52:joker
**********************************/

	int cardImage;	//カードは上コメントのように扱う
...
public:
	...
	Card(void)	{ }
	// 組と番号を設定する
	void set(int suit,int num) {
	  if(suit == Card::SUIT_JOKER) {
	    cardImage = jokerNum;
	  }
	  else {
	    cardImage = suit * 13 + num - 1; 
	  }
	}
	...
	bool equal(Card tgt) { 
           	  return (cardImage == tgt.cardImage); 
	}


	//カードとしておかしいときfalse,正しい時true
	bool isValid() {
	  if ( (-1 < cardImage) && (cardImage < 52) ) {
	          return true;
	  }
