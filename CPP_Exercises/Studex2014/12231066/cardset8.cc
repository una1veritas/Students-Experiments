...
bool Card::scan(void)
{
  ...
  for(int s = SUI T_SPADE; s <= SUIT_CLUB; s++)
    if(!strcmp(b uf, suitnames[s])) {
      suit = s; 
      if(scanf("%d", &number) < 1)
 	return true;
...
{
  for(int i = 0; i < numcard; i++)
    if((number-100)%13 == cdat[i].gnumber())
      return i;
...
bool CardSet::remove(int num){
  if(locate(num) < 0)
    ...
    cdat[locate(num)] = cdat[numcard-1];    
    numcard--;
    return false;
