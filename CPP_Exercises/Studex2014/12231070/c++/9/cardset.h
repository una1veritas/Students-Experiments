...
public:
	...
	bool equal(Card tgt) { 
		return (gsuit() == tgt.gsuit()) && (gnumber() == tgt.gnumber()); 
	}

	bool isValid() {
		if ( ((SUIT_SPADE <= gsuit()) && (gsuit() <= SUIT_CLUB)) 
			 && (1 <= gnumber() && (gnumber() <= 13)) )
			return true;
...
private:
	...
	int locate(int point, int number);
public:
	...
	void print(void);
		// 自身の状態を標準出力に出力する
	int getnumcard(void)	{ return numcard; }
		// access method numcard
