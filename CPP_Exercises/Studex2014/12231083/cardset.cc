...
void Card::print(void)
{
  ...
  if(suit < SUIT_JOKER)
    printf("[%s %d]", suitname[suit], number%13);
...
bool CardSet::remove(Card target){
  int i;
  if(locate(target)!=-1){
    for(i = locate(target);i < this->numcard; i++)
      this->cdat[i] = this->cdat[i + 1];
  this->numcard--;
  return false;
}
  return true;
...
bool CardSet::remove(int num){
  int i;
  if(locate(num)!=-1){
    for(i = locate(num);i < this->numcard; i++)
      this->cdat[i] = this->cdat[i + 1];
    this->numcard--;
    return false;
  }
