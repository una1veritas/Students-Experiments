//
// babastate_7.cc - ババ抜きの状態型(C++版)
//	大高弥  4/26
//
#include "babastate_7.h"
#include <iostream>
...
bool BabaState::move(int from , int to){	
      ...
      std::cout <<"\tplayercard\n";
	  for(int i=0;i<hand[from].num_move();i++){
		     std::cout << i;
	  }
//   std::cout <<"select= ";
//	 std::cin >>number;
// 	 hand[from].pickup(&move,number);
 	 hand[from].pickup(&move,rand()%int(hand[from].num_move())+1);
	 if(hand[to].remove(move.gnumber())){}
     else hand[to].insert(move);
	return true;
