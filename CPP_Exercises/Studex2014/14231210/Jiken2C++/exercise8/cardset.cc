...
void Card::print(void)
{
  ...
  if(suit < SUIT_JOKER)
  {
    //if((number+number/14)%14==0) no=1;
    //else no=(number+number/14)%14;
    if(number%13==0) no=13;
    else no=number%13;
    printf("%d [%s %d]", number, suitname[suit], no);
  }
  else if(suit == SUIT_JOKER)
    printf("%d [joker]",number);
}
