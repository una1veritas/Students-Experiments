...
public:
	...
	int gsuit(void)	{
		int i;
		if(1<=number && number<=13) return SUIT_SPADE;
		if(14<=number && number<=26) return SUIT_DIAMOND;
		if(27<=number && number<=39) return SUIT_HEART;
		if(40<=number && number<=52) return SUIT_CLUB;
		return SUIT_JOKER;
	}
