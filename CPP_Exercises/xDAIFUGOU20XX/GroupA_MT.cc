...
GroupA_MT::GroupA_MT(const char * s) : Player(s) { }

bool GroupA_MT::approve(CardSet & pile, int numCards[]) {
...
bool GroupA_MT::follow(CardSet & pile, CardSet & s) {
	s.makeempty();
...
//並び替え
bool GroupA_MT::sort(CardSet & mine) {
	Card strong;
	int size = mine.size();
	for(int i=0; i < size; i++){
		strong = mine.at(0);
		for(int j=1; j<size-i; j++){
			if(!(mine.at(j).isGreaterThan(strong))){
				strong = mine.at(j);
			}
		}
		mine.remove(strong);
...
void GroupA_MT::calcRemains(CardSet dels){
  Card tmp;
CardSet remain;		//「場に出てないカード」
  remain.makedeck();	//「場に出てないカード」にすべてのカードをセット
  for(int i=0; i < memory.size(); i++){	//「場に出てないカード」から「場に出たカード」を抜く
    remain.remove(memory.at(i));
  }
	dels.print();
  for(int j=0; j < dels.size(); j++){
	remain.remove(dels.at(j));
  }
...
CardSet GroupA_MT::strongestNow(int num){
...
bool GroupA_MT::thinkAndSetCard(CardSet & pile,CardSet & mine,CardSet & s){
	Card tmpCard;
	MyCard tmpMyCard;
		...
		case 1:
			for(int i=0;i<mycards.getSetSize();i++){
				if((!(mycards.at(i).isWait()))&&(mycards.at(i).getLevel()>=1)){
					...
					for(int j=0;j<tmpCardSet.size();j++){
						tmpCard=tmpCardSet.at(j);
						if(tmpCard.isGreaterThan(pile.at(0))){
							s.insert(tmpCard);
							mine.remove(tmpCard);
							return true;
						}
...
bool MyCard::set(Card c){
	...
	for(int i=0;i<cards.size();i++){
		if(cards.at(i).getrank()!=c.getrank()){
			okflag=false;
		}
	}
	if(okflag){
...
bool MyCard::equal(MyCard tgt){
  ...
  for(int i=0;i<getLevel();i++){
    ...
    for(int j=0;j<tgt.getLevel();j++){
      tmp2=tgt.getCards().at(j);
      if(tmp1.equal(tmp2)) count++;
    }
  }
...
bool MyCard::isJoker(){
	if((cards.size()==1)&&(cards.at(0).isJoker())) return true;
	else return false;
}

MyCard MyCardSet::at(int i){
  if(numcard>=i) return cards[i];
}
...
bool MyCardSet::set(CardSet cardset){
	...
	for(int i=0; i<cardset.size(); i++){
		if((cardset.at(i).isGreaterThan(tmp))&&(i!=0)){
		  insert(tmp2);
			tmp2.reset();
		}
		tmp = cardset.at(i);
