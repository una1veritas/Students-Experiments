...
GroupEPlayer1::GroupEPlayer1(const char * s) : Player(s) {}

bool GroupEPlayer1::approve(CardSet & pile, int numCards[]){
  memory.insert(pile);
...
bool GroupEPlayer1::follow(CardSet & pile, CardSet & s){
  	Card tmp[53];
	Card pic;
	Card sub;
	CardSet pass;
...
//手持ちで一番弱いカードを1枚出す
	...
	case 1:	//一枚のとき
		...
		return true;
	      ...
	      if (inHand().at(i).isGreaterThan(pile.at(0)) && (twocards[inHand().at(i).getrank()] != 1) ){//候補を選ぶ(ペアからは選ばない)
		tmp[++count] = inHand().at(i);
	      }
	    }
	    if (count != -1){
	    ...
	    if (count == -1){
	      count == 0;
	      for (int i = 0; i < limit_i; i++){
		if (inHand().at(i).isGreaterThan(pile.at(0))){//候補を選ぶ(ペアからも選ぶ)
		  tmp[++count] = inHand().at(i);
		}
	      }	
	      if (count != -1){
	...
	case 2://2枚だし
	  if(two_flag > 0){
	    for (int i = 0; i < 14; i++){//2ペアで出せるカードの候補を求める
	      sub.set(0,i);
	      if ( (twocards[i] == 1) && (sub.isGreaterThan(pile.at(0))) ){
		ret[i] = PickUpCard(i);//2ペアのカードセットをつくる
		tmp[++count] = ret[i].at(0);
	      }
	    }
	  ...
	  if (three_flag > 0){//3カードを分割して出す
	    for (int i = 0; i < 14; i++){//3カードで出せるカードの候補を求める
	      sub.set(0,i);
	      if ( (threecards[i] == 1) && (sub.isGreaterThan(pile.at(0))) ){
		ret[i] = PickUpCard(i);//3ペアのカードセットをつくる
		tmp[++count] = ret[i].at(0);
	      }
	    }
	  ...
	  if (four_flag > 0){
	    for (int i = 0; i < 14; i++){//4カードで出せるカードの候補を求める
	      sub.set(0,i);
	      if ( (fourcards[i] == 1) && (sub.isGreaterThan(pile.at(0))) ){
		ret[i] = PickUpCard(i);//4ペアのカードセットをつくる
		tmp[++count] = ret[i].at(0);
	      }
	    }
...
Card GroupEPlayer1::min_card(Card c[],int limit){//一番弱いカードを返す
  Card min;
  min.set(4,0);

  for (int i=0;i<limit;i++){
    if(min.isGreaterThan(c[i]))
...
void GroupEPlayer1::SearchCards(void){//揃っているカードを分ける
  int num[14] = {}; //0~13
  int limit = size();
...
CardSet GroupEPlayer1::PickUpCard(int a){//数字で指定されたカードをセットにして返す
  int limit = size();
  CardSet ret;
  for (int i = 0; i < limit; i++){
