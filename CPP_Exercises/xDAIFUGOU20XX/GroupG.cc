 ...
 */
bool GroupG::approve(CardSet & pile, int numCards[]) {

	// new-game flag
	if (inHand().size() > this->myCardSize) {
		this->memory.makeempty();
		this->restCards.makedeck();
	}
 ...
 * @return bool: 成功時:true, 失敗時:false
 */
bool GroupG::follow(CardSet &pile, CardSet &s) {
	int locate = -1;
	int sendSize = 0;
...
int GroupG::getMinCardSize() {
	int min = 100;
	for (int i = 0; this->rivalCardSize[i + 1] != NO_MORE_PLAYERS; i++ ) {
