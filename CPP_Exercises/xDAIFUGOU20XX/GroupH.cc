...
bool GroupH::approve(CardSet & pile, int numCards[]) {
  memory.insert(pile);
...
bool GroupH::follow(CardSet & pile, CardSet & s) {
  Card tmp;
  CardSet tmp1;
...
void GroupH::sort(){
  ...
  for(i = 0; i < inHand().size() ; i++){
    for(j = inHand().size() ; j > i; j--){
      if(inHand().operator[](j-1).isGreaterThan(inHand().operator[](j))){
